...
class _LoginPageState extends ConsumerState<LoginPage> {
  ...
  Future<void> _verifyPin() async {
    ...
    final cashierName = _selectedCashier!.name;
    ...
    final loginUseCase = ref.read(loginWithPinUseCaseProvider);
    final result = await loginUseCase(LoginWithPinParams(
      user: _selectedCashier!,
      pin: _pin,
    ));
    ...
    if (mounted) {
      result.fold(
        ...
        (authenticatedUser) {
          ref.read(activeUserProvider.notifier).setUser(authenticatedUser);
          ToastHelper.showSuccess(
            context,
            'Selamat datang kembali, $cashierName!',
          );
          if (authenticatedUser.isFirstLogin) {
            context.go('/first-time-login');
          } else {
            context.go('/');
          }
        },
      ...
      body: SafeArea(
        child: cashiersAsync.when(
          ...
          data: (cashiers) {
            if (cashiers.isEmpty) {
              return const Center(
                child: Text('Belum ada akun kasir aktif terdaftar.'),
              );
            }

            // Auto-select first cashier if none is selected
            if (_selectedCashier == null) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (mounted && _selectedCashier == null) {
                  setState(() {
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      image: const DecorationImage(
                        image: AssetImage('assets/image/image_1.jpeg'),
                        fit: BoxFit.cover,
                      ),
                    ),
                  ),
                  ...
                  SizedBox(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isSelected = _selectedCashier?.id == cashier.id;
