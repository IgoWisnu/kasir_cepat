...
class _CategoryListPageState extends ConsumerState<CategoryListPage> {
  ...
  void _showFormSheet([Category? category]) {
    showModalBottomSheet(
      ...
      builder: (context) {
        return CategoryFormSheet(editingCategory: category);
      },
    );
  }
  ...
  Future<void> _onDeleteCategory(Category category) async {
    ...
    if (confirmed == true && mounted) {
      final success = await ref.read(categoryListProvider.notifier).deleteCategoryProfile(category.id!);
      if (mounted) {
        if (success) {
          ToastHelper.showSuccess(context, 'Kategori berhasil dihapus!');
        } else {
          ToastHelper.showError(context, 'Gagal menghapus kategori.');
        }
      }
    }
  }
            ...
            child: categoryListState.when(
              ...
              data: (categories) {
                ...
                final filteredCategories = categories.where((category) {
                  final matchesName = category.name.toLowerCase().contains(_searchQuery);
                  final matchesDesc = category.description?.toLowerCase().contains(_searchQuery) ?? false;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Gunakan tombol + di kanan bawah untuk membuat kategori baru.',
